...
class ThyroidRecordController {
    save = async (req, res) => {
        try {
            let newHealthRecord = await ThyroidRecordModel.findById(req.body?._id);
    ...
    getAll = async (req, res) => {
        try {
            const records = (await ThyroidRecordModel.find({}).select('patientInfo typeHealthRecord healthRecordId')).map((record) => {
                return {
    ...
    search = async (req, res) => {
        try {
            ...
            let record = (await ThyroidRecordModel.find({ typeHealthRecord: new RegExp(typeHealthRecord), healthRecordId: new RegExp(healthRecordId) }).select('patientInfo typeHealthRecord healthRecordId')).map((record) => {
                return {
                    fullname: record.patientInfo.fullname,
                    dob: record.patientInfo.dob,
                    typeHealthRecord: record.typeHealthRecord,
